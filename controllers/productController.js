...
exports.getAllProducts = catchAsyncError(async (req, res, next) => {
	...
	const apiFeature = new ApiFeatures(Product.find().populate('category').populate('discount'), req.query)
		.search()
		.filter();
...
exports.getHotProducts = catchAsyncError(async (req, res, next) => {
	const products = await Product.find().sort({ sold: -1 }).populate('category').populate('discount').limit(10);
